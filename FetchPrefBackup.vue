...
const drawChartIG = async (id, name, age) => {
...
const resetChartIG = async () => {
  for (let i = 0; i < insertedGraphs.value.length; i++) {
    emit('removeItems', insertedGraphs.value[i].id)
  }
}

const sendChartIG = async () => {
  for (let i = 0; i < insertedGraphs.value.length; i++) {
    emit(
      'addItems',
      insertedGraphs.value[i].id,
      insertedGraphs.value[i].name,
      insertedGraphs.value[i].population
    )
  }
}
